...
本仓库包含两种运行形态，**共用同一套数据处理逻辑**（`daily_report/function.py` + `daily_report/report_core.py`）：

- **本机 GUI / exe 版**：人工选文件、出结果 Excel（`daily_report/app.py`）
- **云服务器无界面版**：收邮件 → 自动处理 → 出四张图 → 微信推送（`daily_report/server.py`）
...
├── build.py / morning_report.spec / build_windows.bat  # exe 打包
└── .env.example                # 8990 服务配置模板

各模块依赖与入口均在模块内：
  daily_report/requirements.txt          日报服务依赖
  analytics/requirements.txt · main.py   经营分析依赖 + 入口
  company_kb/requirements.txt · api.py    知识库依赖 + 入口
```
...
```bash
# 推荐：Web 服务 + 后台轮询一体
python daily_report/web_server.py

# 其他选项
python daily_report/web_server.py --no-poll   # 仅 Web，不轮询邮件
python daily_report/web_server.py --once      # 启动时立刻收一次
python daily_report/server.py --local A.xlsx B.xlsx  # 跳过邮箱，直接处理本地文件
```
...
**统计口径**（集中在 `zhengqi_visit_stats/processor.py` 顶部常量，改口径只改这里）：
...
```bash
python -m zhengqi_visit_stats.cli 输入.xlsx [输出.xlsx]
```
...
企业微信通报模块接收金山多维表格数据，保存到 `runtime/wecom_notice.db`，按 24 个定时任务向群内发送客户经理个人提醒、简洁通报、详细通报和周通报。6081 由 Nginx 对外提供，FastAPI 只监听本机 8996，避免同一端口冲突。

除周通报（周三 12:15 / 周日 12:00）外所有通知仅周一至周五发送。通报针对「下一个工作日」的预约，周五顺延到下周一，并按周六~周一三天合计考核。详见 [wecom_notice/README.md](wecom_notice/README.md)。
...
```bash
# 临时后台运行（测试用）
nohup python analytics/main.py > runtime/analytics.log 2>&1 &
```
...
ExecStart=/usr/bin/python3 analytics/main.py
Restart=on-failure
RestartSec=5
...
```bash
# 直接处理本地文件（不收邮件）
python daily_report/server.py --local A.xlsx B.xlsx

# OpenClaw 桥接
python daily_report/openclaw_bridge.py latest               # 返回最新处理记录
python daily_report/openclaw_bridge.py date 2026-06-25      # 返回指定日期记录
python daily_report/openclaw_bridge.py ingest A.xlsx B.xlsx # 识别并处理入库
```
...
| 配置项 | 文件 |
|---|---|
| 全光网关套餐配置 | `daily_report/function.py` → `GATEWAY_CONFIG` |
| 政企客户经理名单 | `daily_report/function.py` → `names`（同步更新 `analytics/config.py` → `NAMES`）|
| 高装人员名单 | `daily_report/function.py` → `gaozhuang_names`（同步更新 `analytics/config.py` → `GAOZHUANG_NAMES`）|
...
> **人员名单说明**：`daily_report/function.py` 控制日报生成，`analytics/config.py` 控制经营分析，两处需同步修改。
